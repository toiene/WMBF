...
y_h = test_fun_2(x_h,1);
y_l_1 = test_fun_2(x_l,2);
y_l_2 = test_fun_2(x_l,3);
y_l_3 = test_fun_2(x_l,4);
y_true = test_fun_2(x_test,1);
%%
dmodel_l1 = dacefit(x_l,y_l_1,@regpoly0,@corrgauss2,ones(dim,1),ones(dim,1)*0.01,ones(dim,1)*100);  
dmodel_l2 = dacefit(x_l,y_l_2,@regpoly0,@corrgauss2,ones(dim,1),ones(dim,1)*0.01,ones(dim,1)*100); 
dmodel_l3 = dacefit(x_l,y_l_3,@regpoly0,@corrgauss2,ones(dim,1),ones(dim,1)*0.01,ones(dim,1)*100);

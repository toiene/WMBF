function  [dmodel,perf] = dacefit_ihk2_se(SL, YL, SH, YH, regr, corr, theta0, lob, upb)
...
% Normalize data
mS = mean([SL; SH]);   sS = std([SL; SH]);
mY = mean([YL; YH]);   sY = std([YL; YH]);

sc = (SL - repmat(mS,size(SL,1),1))./repmat(sS,size(SL,1),1);
...
% Regression matrix
[dmodel1, ~] = dacefit(sc, yc, regr, corr, theta0, lob, upb);
ff_p = predictor(se, dmodel1);
...
dmodel = struct('regr',regr, 'corr',corr, 'theta',theta.', ...
  ...
  'C',fit.C, 'Ft',fit.Ft, 'G',fit.G, 'dmc', dmodel1);

...
% Last update August 26, 2002
  ...
  if  isnan(dmodel.beta)   %isnan表示值不是数字
    y = NaN;   
    error('DMODEL has not been found')
  end

  [m n] = size(dmodel.S);  % number of design sites and number of dimensions
  sx = size(x);            % number of trial sites and their dimension 返回一个行向量，该行向量的第一个元素是矩阵的行数，第二个元素是矩阵的列数
  if  min(sx) == 1 & n > 1 % Single trial point 
    nx = max(sx);
    if  nx == n 
      mx = 1;  x = x(:).'; %形成一个列向量后转至成行向量
    end
  else
    mx = sx(1);  nx = sx(2);
  end
  ...
  if  mx == 1  % one site only
    ...
    if  nargout > 1                 % gradient/Jacobian wanted  输出数多于1
      [f df] = feval(dmodel.regr, x);  %feval函数：把已知的数据或符号带入到一个定义好的函数句柄中
      ...
      if  nargout > 2  % MSE wanted  输出数多于2
        rt = dmodel.C \ r;
        u = dmodel.Ft.' * rt - f.';
        v = dmodel.G \ u;
        or2 = repmat(dmodel.sigma2,mx,1) .* repmat((1 + sum(v.^2) - sum(rt.^2))',1,q);
        
        if  nargout > 3  % gradient/Jacobian of MSE wanted  输出数多于3

function  [y, or1, or2, dmse] = predictor_ihk2_extended(x, dmodel)
...
% Last update August 26, 2002
  ...
  if  mx == 1  % one site only
    ...
    if  nargout > 1                 % gradient/Jacobian wanted
      ...
      [ff3 , df3, dff3] = predictor(x, dmodel.dmc_3);
      df = 1./(1./df1 + 1./df2 + 1./df3);
      dff = 1./(1./dff1 + 1./dff2 + 1./dff3);
      ff = dff.*(ff1./dff1 + ff2./dff2 + ff3./dff3);
...
%       dy = (df * dmodel.beta).' + dmodel.gamma * dr;
      ...
      [ff3 , dff3] = predictor(x, dmodel.dmc_3);
      dff = 1./(1./dff1 + 1./dff2 + 1./dff3);
      ff = dff.*(ff1./dff1 + ff2./dff2 + ff3./dff3);
    ...
    [ff3 , dfff3] = predictor(x, dmodel.dmc_3);
    dfff = 1./(1./dfff1 + 1./dfff2 + 1./dfff3);
    ff = dfff.*(ff1./dfff1 + ff2./dfff2 + ff3./dfff3);

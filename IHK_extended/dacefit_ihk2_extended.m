function  [dmodel, perf] = dacefit_ihk2_extended(SL1, YL1, SL2, YL2, SL3, YL3, SH, YH, regr, corr, theta0, lob, upb)
...
% Check design points
[m n] = size(SH);  % number of design sites and their dimension
sY = size(YH);
...
% Normalize data
mS = mean([SL1; SL2; SL3; SH]);   sS = std([SL1; SL2; SL3; SH]);
mY = mean([YL1; YL2; YL3; YH]);   sY = std([YL1; YL2; YL3; YH]);

sc1 = (SL1 - repmat(mS,size(SL1,1),1))./repmat(sS,size(SL1,1),1);
sc2 = (SL2 - repmat(mS,size(SL2,1),1))./repmat(sS,size(SL2,1),1);
sc3 = (SL3 - repmat(mS,size(SL3,1),1))./repmat(sS,size(SL3,1),1);
se = (SH - repmat(mS,size(SH,1),1))./repmat(sS,size(SH,1),1);
yc1 = YL1;
yc2 = YL2;
yc3 = YL3;
ye = YH;
...
for k = 1 : m-1
  ...
  D(ll,:) = repmat(se(k,:), m-k, 1) - se(k+1:m,:); % differences between points
end
if  min(sum(abs(D),2) ) == 0
  error('Multiple design sites are not allowed'), end

% Regression matrix
[dmodel1_1, ~] = dacefit(sc1, yc1, regr, corr, theta0, lob, upb);
[ff_p1, MSE_1] = predictor(se, dmodel1_1);
[dmodel1_2, ~] = dacefit(sc2, yc2, regr, corr, theta0, lob, upb);
[ff_p2, MSE_2] = predictor(se, dmodel1_2);
[dmodel1_3, ~] = dacefit(sc3, yc3, regr, corr, theta0, lob, upb);
[ff_p3, MSE_3] = predictor(se, dmodel1_3);

% [dmodel1_1, ~] = dacefit(SL1, yc1, regr, corr, theta0, lob, upb);
% [ff_p1, MSE_1] = predictor(SH, dmodel1_1);
% [dmodel1_2, ~] = dacefit(SL2, yc2, regr, corr, theta0, lob, upb);
% [ff_p2, MSE_2] = predictor(SH, dmodel1_2);
% [dmodel1_3, ~] = dacefit(SL3, yc3, regr, corr, theta0, lob, upb);
% [ff_p3, MSE_3] = predictor(SH, dmodel1_3);

MSE = 1./(1./MSE_1 + 1./MSE_2 + 1./MSE_3);
ff_p = MSE.*(ff_p1./MSE_1 + ff_p2./MSE_2 + ff_p3./MSE_3);

nn = (n+1)*(n+2)/2;  % Number of columns in f  
% Compute  f
f = [ff_p se zeros(m,nn-n-1)];
for j = 1:m
  f(j,1+(1:n)) = se(j,1:n).*ff_p(j,1);
end
j = n+1;   q = n;
for  k = 1 : n
  f(:,j+(1:q)) = repmat(se(:,k),1,q) .* se(:,k:n).*repmat(ff_p(:,1),1,q);
  j = j+q;   q = q-1;
end
F = f;
...
par = struct('corr',corr, 'regr',regr, 'y',ye, 'F',F, ...
  'D', D, 'ij',ij, 'scS',sS);
...
dmodel = struct('regr',regr, 'corr',corr, 'theta',theta.', ...
  ...
  'S',se, 'Ssc',[mS; sS], 'Ysc',[mY; sY], ...
  'C',fit.C, 'Ft',fit.Ft, 'G',fit.G, 'dmc_1', dmodel1_1, 'dmc_2', dmodel1_2, 'dmc_3', dmodel1_3);

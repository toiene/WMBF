function  [y, or1, or2, dmse] = predictor_hk2_extended(x, dmodel)
...
% Last update August 26, 2002
  ...
  if  mx == 1  % one site only
    ...
    if  nargout > 1                 % gradient/Jacobian wanted
      [f1 , dyy1, df1] = predictor(x0, dmodel.dmc_1);%x0
      [f2 , dyy2, df2] = predictor(x0, dmodel.dmc_2);
      [f3 , dyy3, df3] = predictor(x0, dmodel.dmc_3);
      dyy = 1./(1./dyy1 + 1./dyy2 + 1./dyy3);
      df = 1./(1./df1 + 1./df2 + 1./df3);
      f = df.*(f1./df1 + f2./df2 + f3./df3);
      
      [r, dr] = feval(dmodel.corr, dmodel.theta, dx);
      % Scaled Jacobian
      dy = (dyy * dmodel.beta).' + dmodel.gamma * dr;
      % Unscaled Jacobian
      ...
      if  nargout > 2  % MSE wanted
        ...
        if  nargout > 3  % gradient/Jacobian of MSE wanted
          ...
          Gv = dmodel.G' \ v;
          g = (dmodel.Ft * Gv - rt)' * (dmodel.C \ dr) - (dyy * Gv)';
          % Unscaled Jacobian
      ...
      [f1 , df1] = predictor(x0, dmodel.dmc_1);%x0
      [f2 , df2] = predictor(x0, dmodel.dmc_2);
      [f3 , df3] = predictor(x0, dmodel.dmc_3);
      df = 1./(1./df1 + 1./df2 + 1./df3);
      f = df.*(f1./df1 + f2./df2 + f3./df3);
